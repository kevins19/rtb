...
def run_test(portion = 1/64, bids = bids_test, ctr = ctr_test, clicks = clicks_test, window_portion = 0.02, bid_factor = 0.4, winrate_factor = 0.003):
    ...
    def get_bid_price(i):
        ...
        # bidding when the estimated cost is below the line
        # perhaps bid above the line
        if dif < 0:
            return (min(K * ctr[i], B), 1)
        
        # bidding when the estimated cost is above the line
        else: 
